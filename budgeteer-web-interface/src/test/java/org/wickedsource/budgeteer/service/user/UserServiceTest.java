...
class UserServiceTest extends ServiceTestTemplate{
    ...
    void testAddUserToProjectSuccess() {
        when(userRepository.findById(1L)).thenReturn(Optional.ofNullable(createUserEntity()));
        when(projectRepository.findById(1L)).thenReturn(Optional.ofNullable(createProjectEntity()));
    ...
    void testGetUsersNotInProject() {
        when(userRepository.findNotInProject(1L)).thenReturn(Collections.singletonList(createUserEntity()));
    ...
    void testGetUsersInProject() {
        when(userRepository.findInProject(1L)).thenReturn(Collections.singletonList(createUserEntity()));
    ...
    void getUserByIdUserIdNotFoundException() {
        Assertions.assertThrows(UserIdNotFoundException.class, () -> {
            when(userRepository.findById(2L)).thenReturn(Optional.ofNullable(createUserEntity()));
